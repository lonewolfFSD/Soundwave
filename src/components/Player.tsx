...
const Player: React.FC = () => {
  ...
  const [desktopSidebarTab, setDesktopSidebarTab] = useState<'queue' | 'lyrics'>('queue');
...
useEffect(() => {
  ...
  useEffect(() => {
    if (isDesktopFullScreen && desktopSidebarTab === 'lyrics' && activeLineIndex !== -1 && !isUserScrolledDesktop) {
...
// --- NATIVE EVENT LISTENERS ---
    ...
    {/* DESKTOP FULL SCREEN PLAYER */}
      ...
      `}>
        ...
        <div className="absolute top-0 left-0 right-[350px] z-20 p-8 flex justify-between items-center shrink-0 pointer-events-none">
          ...
          <div className="flex items-center gap-1.5 backdrop-blur-md bg-black/20 p-1.5 rounded-xl border border-white/5 pointer-events-auto">
  {/* MUSIC BUTTON */}
  <button 
    onClick={() => { triggerHaptic(); setIsVideoMode(false); }} 
    className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 text-sm font-bold ${!isVideoMode ? `${activeColor} bg-white/10 shadow-lg` : `${textMuted} hover:${textMain} hover:bg-white/5`}`}
  ...
  {youtubeId && (
    <button 
      onClick={() => { triggerHaptic(); setIsVideoMode(true); }}
      className={`px-4 py-2 rounded-lg transition-all flex items-center gap-2 text-sm font-bold ${isVideoMode ? `${activeColor} bg-white/10 shadow-lg` : `${textMuted} hover:${textMain} hover:bg-white/5`}`}
      style={{ fontFamily: 'Space Grotesk, sans-serif' }}
    >
      <Video size={16} />
      Video
        ...
        <div className="relative z-10 flex-1 flex flex-row w-full h-full overflow-hidden">
          ...
          {/* LEFT AREA: Media Display */}
          <div className="flex-1 flex flex-col items-center justify-center h-full pb-[100px]">
  <div className="w-full max-w-[750px] flex flex-col items-center gap-8">
    {/* Media Container */}
    {isVideoMode && youtubeId ? (
      <div className="w-full aspect-video rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-white/15 bg-black flex items-center justify-center relative">
        <SyncedVideoPlayer
          videoId={youtubeId}
          isPlaying={isPlaying}
      ...
      <div className={`relative w-[400px] aspect-square rounded-full shadow-2xl transition-all duration-700 flex items-center justify-center bg-gradient-to-br from-zinc-800 to-black border-[12px] border-zinc-900/80 ${isPlaying ? 'scale-100 drop-shadow-[0_0_30px_rgba(255,255,255,0.05)]' : 'scale-90 opacity-80'}`}>
        ...
        >
          ...
          <div className="w-[350px] shrink-0 h-full bg-black/60 backdrop-blur-3xl border-l border-white/10 flex flex-col pb-[100px]">
            ...
            {/* TABS HEADER */}
            <div className="flex text-xs uppercase tracking-widest border-b border-white/10 shrink-0 mt-8">
              <button 
                onClick={() => setDesktopSidebarTab('queue')}
                style={{ fontFamily: 'Space Grotesk, sans-serif' }}
                className={`flex-1 py-4 text-center transition-colors border-b-2 font-bold ${desktopSidebarTab === 'queue' ? `${activeColor} border-current` : `${textMuted} border-transparent hover:${textMain}`}`}
              >
                Queue List
              </button>
              <button 
                onClick={() => setDesktopSidebarTab('lyrics')}
                style={{ fontFamily: 'Space Grotesk, sans-serif' }}
                className={`flex-1 py-4 text-center transition-colors border-b-2 font-bold ${desktopSidebarTab === 'lyrics' ? `${activeColor} border-current` : `${textMuted} border-transparent hover:${textMain}`}`}
              >
                Lyrics
              </button>
            </div>
  ...
  {/* TAB 1: ALL SONGS (QUEUE) */}
 {desktopSidebarTab === 'queue' && (
   <div className="flex flex-col gap-4 p-3"> {/* Increased gap to separate the two sections */}
          ...
          return (
            ...
            >
              ...
              {isActive && isPlaying ? (
                ...
                // SHOW CHECKMARK IF ADDED (Syncs perfectly with theme activeColor)
                <div className={`p-1.5 rounded-md opacity-100 transition-all ${activeColor}`} title="Added to Queue">
                  <Check size={18} />
                </div>
              ) : (
                // SHOW PLUS BUTTON IF NOT ADDED
                <button 
                  onClick={(e) => { 
                    e.stopPropagation(); 
                    addToQueue(song); 
  ...
  {desktopSidebarTab === 'lyrics' && (
    <div className="absolute inset-0 w-full h-full flex flex-col relative">
      <div 
        ...
        className="flex-1 w-full p-6 text-center mask-image-gradient overflow-y-auto scrollbar-hide"
        style={{
      ...
      >
        {parsedLyrics.length > 0 ? (
          parsedLyrics[0].time !== -1 ? (
            <div className="space-y-2 pt-10 pb-32 px-3 md:px-6">
            ...
            <p className={`text-base text-left font-bold leading-relaxed whitespace-pre-wrap font-sans ${lyricsActive} pt-4 pb-16`} style={{ fontFamily: 'Space Grotesk, sans-serif' }}>
              {currentSong.lyrics}
            </p>
          )
        ) : (
          <div className={`flex flex-col items-center justify-center h-full gap-4 ${lyricsMuted}`}>
            <p className="text-[13px] font-medium" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>No lyrics found.</p>
          </div>
        )}
      </div>
